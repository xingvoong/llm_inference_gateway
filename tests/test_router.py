...
def test_high_priority_routes_to_best_model():
    provider, model = route_request(prompt="tell me something", priority="high")
    assert isinstance(provider, OpenAIProvider)
    assert "gpt" in model


def test_low_cost_routes_to_fast_model():
    provider, model = route_request(prompt="tell me something", max_cost=0.005)
    assert isinstance(provider, HuggingFaceProvider)


def test_priority_overrides_cost():
    # even with low cost, high priority should win
    provider, model = route_request(prompt="tell me something", priority="high", max_cost=0.001)
    assert isinstance(provider, OpenAIProvider)
    assert "gpt" in model


def test_no_params_returns_default():
    # long prompt, no priority, no cost → default model
    prompt = "a" * 200
    with patch("app.router.is_trained_model_available", return_value=False):
        with patch("app.router.classify_prompt", return_value="general chat"):
            provider, model = route_request(prompt=prompt)
            assert isinstance(provider, OpenAIProvider)


def test_code_prompt_routes_to_fast_model():
    prompt = "a" * 200  # long enough to skip cost/length rules
    with patch("app.router.is_trained_model_available", return_value=False):
